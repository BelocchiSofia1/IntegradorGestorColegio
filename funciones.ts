...
//JSON.parse significa analizar y convertir un programa en un formato interno que un entorno de ejecución pueda realmente ejecutar
...
//El spread operator operador que simplifica la recogida de valores en una estructura de datos. Convierte un array o un objeto en el conjunto de valores que contiene.
...
function preguntarDato (entidad:string, dato:string):string{
    let informacion:string = readLineSync.question(`Escriba el ${dato} del ${entidad}: `).toLocaleLowerCase();
    return informacion
}
...
function seleccionarMaterias(curso:Curso, indicacion:string):Materia[] {
    ...
    while (condicion != -1) {
        ...
        if(materia!== -1 && nombredeLasMaterias[materia] != `---`){
            ...
            console.log(`Se ha matriculado a ${nombredeLasMaterias[materia]}`);
            console.clear();
            nombredeLasMaterias.splice(materia,1,"---")
            //splice permite cambiar el contenido del arreglo eliminando o sustituyendo
            }else {
                console.clear();
                console.log("Ya se matriculo a esa materia, seleccione una opcion valida");
            }
        condicion=materia
    }
...
//creo una funcion para reutilizar el codigo en alumnos y profesores, esta recibe por parametro una entidad (alumno o profesor) y una direccion del archivo JSON:
function eliminarEntidad(entidad:string, direccionArchivo:string){
    // verifico si existe el archivo (la funcion retorna true o false)
    if(verificarExitencia(direccionArchivo)){
        ...
        if(entidad.toLocaleLowerCase()=="alumno"){
            ...
            //findIndex retorna el primer elemento encontrado de un arreglo o matriz 
        ...
        // el metodo findIndex busca en el listado de entidades(alumnos o profesores) a la entidad que coincida con el ID brindado(nos devuelve su indice).
        // si encuentra a la entidad:
        if (eliminarEntidad >= 0){
            console.clear();
            console.log(`${entidad} encontrado con EXITO`);
            // compruebo que quiera realmente eliminar a la entidad
            let opciones:string[] = ["SI","NO"];
            console.log(`ESTA SEGURO QUE QUIERE ELIMINAR AL ${entidad} CON LA ID: ${idEntidad} (${listadoEntidades[eliminarEntidad].nombre}, ${listadoEntidades[eliminarEntidad].apellido})`);
            ...
            switch (numero) {
                case -1:
                    ...
                    console.clear();
                    console.log("Volviendo al menu de inicio...");
                    console.log("");
                    break;
                case 0:
                    ...
                    console.clear();
                    console.log(`El ${entidad} con la ID: ${idEntidad} HA SIDO ELIMINADO CON EXITO`);
                    console.log("");
        ...
        // si NO encuentra a la entidad(-1):
             ...
             console.clear();
             console.log(`El ${entidad} con el ID: ${idEntidad} NO HA SIDO ENCONTRADO O HA INGRESADO UN ID INCORRECTO`);
             console.log("");
        ...
        console.log(`NO HAY ${entidad} CREADOS PARA DAR UN LISTADO Y ELIMINAR`);
        console.log("");
    }
}
...
function crearlistaDeUnaEntidad(entidad:string, direccionlistadoDeEntidades:string){
    ...
    if(verificarExitencia(direccionlistadoDeEntidades)){
        ...
        let idEntidad:string= preguntarDato(entidad,"ID");
            ...
            if(entidadEncontrada == -1){
                console.log("");
                console.log(`NO se encontro el alumno con el ID: ${idEntidad}`);
                console.log("");
            }else {
                console.log("");
                console.log(`Se ha encontrado el ${entidad} y ha sido exportado a la siguiente direccion: ./Listados/${listadoEntidades[entidadEncontrada].nombre}-${listadoEntidades[entidadEncontrada].apellido}.json`);
                console.log("");
    ...
    console.log("");
    // en el caso de que no exista el archivo:
    console.log(`NO HAY ${entidad.toLocaleUpperCase()} CREADOS PARA DAR UN LISTADO`);
    console.log("");
}
}
